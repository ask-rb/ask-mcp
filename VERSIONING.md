...
ask-mcp is pre-1.0 (`0.y.z`). Until 1.0.0:

- **PATCH** (`0.6.0` → `0.6.1`) — bug fixes and other
  backwards-compatible corrections. No new features, no public API changes.
- **MINOR** (`0.6.0` → `0.7.0`) — new features **and**
...
Patch numbers increase by exactly one per release. From `0.6.0` the next
patch is `0.6.1` — never `0.6.3`, never a skip, never a jump. Patch
...
Every `ask-*` gem — ask-mcp included — and `yamine` is released through
...
gemchain guard ask-mcp                       # blast radius: who depends on us
gemchain update ask-mcp 0.6.1 --dry-run
gemchain update ask-mcp 0.6.1 --test-only
gemchain update ask-mcp 0.6.1
```
...
2. **Version** — bump `lib/ask/mcp/version.rb` by exactly one step
   (gemchain does this).
...
5. **Build** — `gem build ask-mcp.gemspec` succeeds and the packaged file
   list looks right.
...
- `lib/ask/mcp/version.rb` at `HEAD`
- the `vX.Y.Z` git tag
...
- Fix a bug with no API change → `0.6.1` (patch).
- Add a new MCP capability → `0.7.0` (minor).
- Remove or rename a public method → `0.7.0` (minor — breaking is allowed
  pre-1.0, but only ever in a minor).
- From `0.6.0`, **never** jump to `0.6.3` for a fix, and **never** release
  `0.8.0` for a one-line bugfix.
- Two fixes in a row → `0.6.1`, then `0.6.2` — one step each time.
- Dependency example: releasing `ask-core` runs
  `gemchain update ask-core <ver>` from the workspace root; the cascade
  rewrites this gem's gemspec constraint, re-runs this suite, bumps ask-mcp
  by one patch, commits, publishes, tags, and pushes — no manual steps in
  this repository.
